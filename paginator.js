...
function paginatorHelper(options = {}) {
  ...
  function pageLink(i) {
    return `<a class="page-number" href="${link(i)}index.html">${transform ? transform(i) : i}</a>`;
  }

  // Display the link to the previous page
  if (prevNext && current > 1) {
    result += `<a class="extend prev" rel="prev" href="${link(current - 1)}index.html">${prevText}</a>`;
  }
  ...
  if (prevNext && current < total) {
    result += `<a class="extend next" rel="next" href="${link(current + 1)}index.html">${nextText}</a>`;
  }
